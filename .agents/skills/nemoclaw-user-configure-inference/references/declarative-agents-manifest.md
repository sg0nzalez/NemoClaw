...
- [Switch Inference Providers](switch-inference-providers.md) — swap the primary onboard provider before changing per-agent `model` refs, since refs must share that provider.
- Workspace Files (use the `nemoclaw-user-manage-sandboxes` skill) — how per-agent `workspace-<id>` directories are provisioned and persisted across rebuilds.
