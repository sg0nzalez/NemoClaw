...
| Recommendation | Run on Ubuntu 22.04 LTS or another system with Landlock enabled on a kernel that supports it, and treat a Deep Agents Code startup failure as an unsupported runtime or policy-path error instead of changing its policy to `best_effort`. |
