...
`$$nemoclaw update` tracks the maintained `lkg` tag by default, but it honors the same `NEMOCLAW_INSTALL_TAG` (or `NEMOCLAW_INSTALL_REF`) override so you can opt into a specific release even when `lkg` has not been promoted yet:

```bash
NEMOCLAW_INSTALL_TAG=v0.0.60 $$nemoclaw update --yes
```
