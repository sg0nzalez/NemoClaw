...
The strict pre-upgrade backup checks the top level of `/sandbox` for entries outside the image-managed entries and the agent config root.
If the installer reports an unpreserved path, move or download that path and rerun the installer.

<Warning>
Set `NEMOCLAW_ACCEPT_UNPRESERVED_SANDBOX_DATA=1` only when you intend to discard every reported path.
The setting acknowledges the loss and does not include those paths in the snapshot.
</Warning>

Pass the setting to the installer on the `bash` side of the pipeline:

```bash
curl -fsSL https://www.nvidia.com/nemoclaw.sh | NEMOCLAW_ACCEPT_UNPRESERVED_SANDBOX_DATA=1 bash
```
