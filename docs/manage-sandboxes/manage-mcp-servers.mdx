...
Pass `--tools` with one server name to make a live MCP connection and list the tool names that the server advertises:
...
Tool discovery uses the same generated policy, provider attachment, credential placeholder, proxy environment, and adapter runtime ancestry as the selected agent.
It is available consistently for managed OpenClaw, Hermes, and LangChain Deep Agents Code sandboxes.

The result is a point-in-time list from the MCP server, not an attestation of the exact tools visible to the model in an active agent session.
...
Cleanup errors do not replace the bounded, credential-safe discovery result.
...
Discovery is opt-in because it sends network traffic to the configured MCP endpoint.
