...
During installer-driven pre-upgrade recreation of a legacy OpenClaw sandbox, NemoClaw also restores a missing managed messaging-channel block, such as Slack, from the sanitized pre-upgrade config when the legacy registry cannot regenerate it.
A channel block generated by the new image still wins, and backup sanitization still excludes raw credentials.
...
Before an installer-driven upgrade, strict `backup-all` also checks the top level of `/sandbox` for entries outside the image-managed entries and the agent config root.
If it finds an unpreserved path, the installer exits before it recreates the sandbox.
Move or download the reported path before you retry the upgrade.
Setting `NEMOCLAW_ACCEPT_UNPRESERVED_SANDBOX_DATA=1` explicitly accepts loss of the reported paths, but it does not add those paths to the snapshot.
This strict check does not change a standalone `$$nemoclaw backup-all` run.
