...
For an OpenClaw clone, NemoClaw assigns a dashboard port that differs from the source sandbox's port.
After restoring the snapshot, NemoClaw restarts the clone's gateway, checks its dashboard port forward, and re-establishes the forward when needed before verifying readiness.
If that recovery fails, the command returns a nonzero status and retains the clone and snapshot for diagnosis.
