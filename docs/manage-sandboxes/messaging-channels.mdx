...
The cleanup selects one OpenShell mutation transport and dispatches the deletion once.
It does not fall back to SSH or replay the deletion when the result is inconclusive, because the first dispatch may already have removed the state.
If the sandbox is stopped, its gateway is unreachable, or success cannot be confirmed, the command exits non-zero and asks you to start the sandbox and re-run `channels remove`.
NemoClaw deliberately leaves the registry, policy preset, and `session.policyPresets` unchanged on that failure path, so the re-run reconciles the removal cleanly.
