...
When a sandbox with managed MCP servers cannot run a pre-mutation no-op, explicit `--force` uses the exact host-side registry and provider identities to preserve MCP intent without scrubbing the unreachable in-sandbox adapter.
This recovery remains fail-closed for ambiguous ownership, target, policy-registration, or provider state; an error after a successful no-op does not fall back to the host-side path.
</Warning>
