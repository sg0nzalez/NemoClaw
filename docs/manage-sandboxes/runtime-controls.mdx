...
Custom agents that run a manifest recovery script do not use this controller probe and continue to poll ordinary gateway health through the supported OpenShell sandbox exec API.
If that API is unavailable, recovery fails closed instead of retrying the script or health probe over SSH.
...
- [Switch Inference Providers](../inference/manage-inference/switch-providers) for the rebuild path for provider and model changes.
...
- [Manage Sandbox Lifecycle](lifecycle) for the full rebuild, re-onboard, and upgrade workflow.
- [Switch Inference Providers](../inference/manage-inference/switch-providers) for the runtime route and rebuild paths for provider and model changes.
- [Customize Network Policy](../network-policy/customize-network-policy) and [Approve Network Requests](../network-policy/approve-network-requests) for runtime policy editing and operator approval flow.
- [Security Best Practices](../security/best-practices) for the per-attack-surface posture table that this page complements.
