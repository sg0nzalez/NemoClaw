...
| `--keep-openshell` | Leave OpenShell binaries installed. NemoClaw still disables and removes its own marked `~/.config/systemd/user/openshell-gateway.service` user unit when present, and never removes upstream package-managed OpenShell service units. |
