...
|----------|--------|---------------|-------|
| NVIDIA Endpoints | Tested | OpenAI-compatible | Hosted models on inference.nvidia.com |
...
|--------|-------------|----------------|
| NVIDIA Endpoints | Routes to models hosted on [inference.nvidia.com](https://inference.nvidia.com). You can also enter any model ID from the catalog. Set `NVIDIA_API_KEY`. | Nemotron 3 Super 120B, Nemotron 3 Ultra 550B, GLM-5.1, MiniMax M2.7, GPT-OSS 120B, DeepSeek V4 Pro |
...
|---|---|---|
| `inference.nvidia.com` (NVIDIA-hosted) | **Option 1: NVIDIA Endpoints** | NemoClaw sets the base URL to `https://inference.nvidia.com/v1` for you and validates the model against the NVIDIA endpoint catalog. |
...
models:
  - name: nano
    ...
    api_base: "https://inference.nvidia.com"
...
| NVIDIA Endpoints | Validates through `/v1/chat/completions` only; NemoClaw skips the `/v1/responses` probe because NVIDIA Endpoints does not expose `/v1/responses` (returns 404 for every model). |
