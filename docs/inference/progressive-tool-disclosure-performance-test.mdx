...
`max_attempts` is a bounded per-decomposition retry count, and `timeout_ms` applies to each remote request; both are recorded, and every attempt contributes to reported usage and latency.
