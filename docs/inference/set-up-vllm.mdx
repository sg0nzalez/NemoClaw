...
An inconclusive model-cache check is guarded like an insufficient result because a registered model download can be hundreds of gigabytes: interactive setup requires explicit confirmation, and non-interactive setup stops unless the disk-space override is set.
...
They also do not accept an inconclusive model-cache warning.
Non-interactive setup stops for verified insufficient capacity or inconclusive model-cache capacity unless you set `NEMOCLAW_IGNORE_VLLM_DISK_SPACE=1` exactly.
The override applies when NemoClaw verifies that Docker image storage or model-cache storage is insufficient and when model-cache capacity cannot be verified.
Use it only after you independently verify that the relevant filesystem can complete the download.
An inconclusive Docker image-storage check retains its existing automatic-continue behavior and does not require the override.
</Warning>
