...
- [Set Up Task-Specific Sub-Agents](set-up-sub-agent) — in-sandbox path that edits `agents.list` directly without a rebuild.
- [Switch Inference Providers](switch-inference-providers) — swap the primary onboard provider before changing per-agent `model` refs, since refs must share that provider.
- [Workspace Files](../manage-sandboxes/workspace-files) — how per-agent `workspace-<id>` directories are provisioned and persisted across rebuilds.
