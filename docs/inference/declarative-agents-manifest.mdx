...
- Use [Switch Inference Providers](switch-inference-providers) before changing per-agent `model` refs, since refs must share the primary onboard provider.
- Use [Workspace Files](../manage-sandboxes/workspace-files) to learn how per-agent `workspace-<id>` directories are provisioned and persisted across rebuilds.
