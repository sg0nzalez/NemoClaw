...
The authoritative `status` and `doctor` route probe runs from inside the sandbox and reports HTTP `100` through `499` as `healthy`, HTTP `500` through `599` as `unhealthy`, and `000`, a transport failure, or an unavailable probe as `unreachable` or `not probed`.
