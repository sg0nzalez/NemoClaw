...
sidebar-title: "Use Ollama"
description: "Install, connect, and operate Ollama as NemoClaw's local inference provider."
description-agent: "Shows how to use Ollama as NemoClaw's local inference provider, including installation, routing, model selection, and verification."
keywords: ["nemoclaw local inference", "ollama nemoclaw", "ollama model server"]
content:
  type: "how_to"
skill:
  priority: 10
...
NemoClaw can route inference to Ollama running on your machine instead of a cloud API.
This page covers the Ollama installation, routing, model-selection, and verification workflow.
...
- Ollama running on the host, or a host where the NemoClaw onboard wizard can install or start Ollama.
...
Each Ollama-backed OpenClaw agent passthrough checks the registered route to see whether the selected model is still loaded.
...
Refer to [Set Up vLLM](set-up-vllm) for the server setup flow.
Refer to [Tool Calling Reliability](../tool-calling-reliability) to diagnose tool-call output.
</Warning>
</AgentOnly>
...
Use this configuration to run Ollama onboarding without prompts.
...
The output shows the Local Ollama provider label and the active model.
...
Change the Ollama model without re-running onboard.
...
- [Set Up Self Hosted Inference Servers](.) for an overview of self-hosted options other than Ollama.
- [Set Up vLLM](set-up-vllm) for local vLLM setup.
- [Set Up NVIDIA NIM](set-up-nvidia-nim) for local NVIDIA NIM setup.
- [Other Self Hosted Servers](set-up-other-self-hosted-servers) for a compatible endpoint.
- [Verify Self Hosted Inference](verify-self-hosted-inference) for post-onboarding checks and timeout configuration.
