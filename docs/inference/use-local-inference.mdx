...
Host-side proxy liveness diagnostics treat an HTTP response such as `401` as proof that the proxy answered.
The authoritative `status` and `doctor` route probe runs from inside the sandbox and reports HTTP `200` through `499` as `healthy`, HTTP `500` through `599` as `unhealthy`, and an interim `100` through `199`, `000`, a transport failure, or an unavailable probe as `unreachable` or `not probed`.
