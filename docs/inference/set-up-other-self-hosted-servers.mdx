...
title: "Set Up Other Self Hosted Servers"
sidebar-title: "Other Self Hosted Servers"
description: "Connect NemoClaw to OpenAI-compatible, Anthropic-compatible, and raw model inference servers."
description-agent: "Shows how to configure compatible inference endpoints, raw GGUF model servers, API validation, and API path selection."
keywords: ["nemoclaw compatible endpoint", "nemoclaw anthropic compatible endpoint", "nemoclaw gguf", "nemoclaw llama cpp"]
content:
  type: "task"
---
import { AgentOnly } from "../_components/AgentGuide";

Use this page when you operate a self-hosted server other than NemoClaw-managed vLLM or NVIDIA NIM.
It covers OpenAI-compatible servers, Anthropic-compatible servers, raw GGUF model servers, and endpoint validation.
...
The Ollama local path selects Ollama model tags.
It does not accept a raw `.gguf` file path directly.
...
Start your OpenAI-compatible server first.
The endpoint in `NEMOCLAW_ENDPOINT_URL` must already be serving, or onboarding fails its validation probe.
...
<Warning>
Reasoning mode validates only `/v1/chat/completions`.
It does not verify tool calling or streaming.
...
| `NEMOCLAW_REASONING` | Set to `true` for a reasoning-only compatible endpoint model; accepted values are `true`, `1`, `yes`, and `y` case-insensitively, and the default is `false`. |
| `COMPATIBLE_API_KEY` | Use an API key for the endpoint, or any non-empty value if authentication is not required. |
