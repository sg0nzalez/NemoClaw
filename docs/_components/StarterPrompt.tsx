...
\`\`\`shell
<absolute-node> --experimental-strip-types <private-dir>/local-credential-helper.mts --execution-profile isolated --form <private-dir>/local-credential-form.html --field NVIDIA_INFERENCE_API_KEY:secret -- <absolute-approved-executable> <approved-args...>
\`\`\`

- Before opening the form, inspect the approved executable's first line when it is a script. A launcher such as \`#!/usr/bin/env node\` still needs \`node\` on the helper's sanitized search path even when the launcher itself is absolute. Resolve the interpreter to an absolute executable, include it in the command explanation, and invoke the launcher through that absolute interpreter, for example \`<absolute-node> <absolute-launcher> <approved-args...>\`. Prefer invoking each child tool by absolute executable path instead of \`PATH\` lookup. If a macOS or Linux command cannot avoid a search path, use absolute \`/usr/bin/env\` with one explicit \`PATH=<approved-directories>:/usr/bin:/bin\` argv entry before the absolute interpreter. Every added directory must be an absolute, trusted system or managed-toolchain directory that is not writable by the user, coding agent, project, or workspace. Ask my permission for every directory. Never silently inherit the ambient \`PATH\` or collect \`PATH\` as a form field. If the interpreter or a trusted child tool is unavailable by absolute path, stop before serving the form and explain what is missing.
- Start the helper as a long-running process. Do not wait for the helper process to exit before presenting the URL; it cannot exit until the form is submitted or expires. If the coding-agent command runner streams output, present the URL as soon as it appears. If it buffers output from long-running commands, start the helper without waiting for completion and redirect stdout and stderr to separate files inside the private temporary directory. Restrict those files to the current user, with mode \`0600\` on macOS or Linux or an equivalent current-user-only ACL on Windows. Read only the first matching one-time URL from the stdout file while the helper is still running; do not display the remaining file contents. Keep monitoring the live process, treat both files as sensitive, and delete them after the helper finishes.
...
- Never reuse an old form tab or one-time URL. If the helper ended, the page expired, or the browser reports an unknown outcome, close that tab, check the command result, and start a fresh helper session only when needed.
...
- Treat credential acceptance and command execution as separate phases. Say \`Credential submission accepted locally.\` only after the helper accepts the request; that does not prove that the approved command succeeded. Report command launch, onboarding, and command exit failures separately, and never describe a missing interpreter or later endpoint-validation error as a form failure.
