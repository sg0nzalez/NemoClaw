...
- Before opening the form, inspect the approved executable's first line when it is a script. A launcher such as \`#!/usr/bin/env node\` still needs \`node\` on the helper's sanitized search path even when the launcher itself is absolute. Resolve the interpreter to an absolute executable, include it in the command explanation, and invoke the launcher through that absolute interpreter, for example \`<absolute-node> <absolute-launcher> <approved-args...>\`. If a macOS or Linux command genuinely needs a search path for child tools, use absolute \`/usr/bin/env\` with one explicit \`PATH=<approved-directories>:/usr/bin:/bin\` argv entry before the absolute interpreter, and ask my permission for every directory. Never silently inherit the ambient \`PATH\` or collect \`PATH\` as a form field. If the interpreter is unavailable, stop before serving the form and explain what is missing.
