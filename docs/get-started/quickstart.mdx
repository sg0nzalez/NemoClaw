...
<AccordionGroup>
  ...
  <Accordion title="Installer Behavior and Platform Details">
    ...
    It requires the NVIDIA CDI device `nvidia.com/gpu=all`, using the packaged refresh service or NVIDIA's transient fallback, and verifies CDI and `--gpus all` with real container launches.
