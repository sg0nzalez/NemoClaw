...
| DGX OS (Station) | Docker | Deferred | Deferred. For evaluation on a qualifying DGX Station GB300, see [Additional Setup for DGX Station](prerequisites/dgx-station-preparation) before the Quickstart. |
...
| Windows WSL2 | Docker Desktop (WSL backend) | Tested with limitations | Requires WSL2 with Docker Desktop backend. See [Additional Setup for Windows Machines](prerequisites/windows-preparation) before the Quickstart. |
...
If you are evaluating a qualifying DGX Station GB300, follow [Prepare DGX Station to Install NemoClaw](prerequisites/dgx-station-preparation) before the Quickstart.

### Windows Preparation

If you are using Windows, follow [Prepare a Windows Machine to Install NemoClaw](prerequisites/windows-preparation) before the Quickstart.
