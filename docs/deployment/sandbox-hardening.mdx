...
The NemoClaw sandbox policy uses `compatibility: best_effort` at the OpenShell policy layer, and NemoClaw onboarding separately fails closed when it detects a host or Docker VM kernel that cannot support Landlock.
...
For production deployments, kernel 5.13+ with Landlock enabled is required by onboarding.
