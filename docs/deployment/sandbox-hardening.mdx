...
The Deep Agents Code policy uses `compatibility: hard_requirement`, so OpenShell aborts sandbox startup instead of running the agent with reduced filesystem isolation.
...
A successful ruleset reports `Landlock filesystem sandbox available`, `Applying Landlock filesystem sandbox`, and `Landlock ruleset built` events.
When Landlock is unavailable, `best_effort` emits a high-severity finding and continues, while the same condition prevents Deep Agents Code from starting.
