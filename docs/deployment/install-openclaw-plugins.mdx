...
The linked install preserves existing managed plugin load paths, appends the copied plugin path, refreshes the plugin registry, and records the install before the explicit enable and inspect steps.
