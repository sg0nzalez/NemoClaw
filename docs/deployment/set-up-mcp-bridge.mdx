...
A detail containing `CONNECT 503` means OpenShell failed closed before TLS setup because gateway TLS termination or credential rewriting was unavailable.
Inspect the OpenShell gateway's ephemeral CA initialization and provider environment, repair the reported condition, and rerun `mcp status <server>`.
