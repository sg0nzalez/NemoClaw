...
| Landlock failure handling varies by agent | OpenClaw and Hermes continue with reduced filesystem isolation when Landlock cannot be fully applied, while Deep Agents Code fails sandbox startup. | Run on kernel 5.13 or later with Landlock enabled, resolve any policy-path error reported for Deep Agents Code, keep its policy at `hard_requirement`, and refer to [Landlock LSM Enforcement](../security/best-practices#landlock-lsm-enforcement). |
