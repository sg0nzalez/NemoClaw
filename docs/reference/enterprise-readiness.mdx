...
| Landlock filesystem enforcement requires supported kernels | NemoClaw onboarding aborts before creating or reusing a sandbox when it detects a host or Docker VM kernel below Linux 5.13. | Run on kernel 5.13 or later with Landlock enabled. See [Landlock LSM Enforcement](../security/best-practices#landlock-lsm-enforcement). |
