...
An operator can persistently exclude a specific baseline entry with [`policy exclude`](#excluding-a-baseline-entry) when they accept reduced, unsupported functionality; excluded entries are the only baseline changes that survive rebuild.
...
If a later release changes or removes that entry, create and rebuild fail closed and require re-review.
