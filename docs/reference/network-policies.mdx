...
|------|------------------|-------------|
| Restricted | None | Base sandbox only. No third-party network access beyond inference and core agent tooling. Agent-required preset additions, such as OpenClaw pricing fetches, are suppressed; reapply them later with `policy-add` if cost recording or other agent-side features are needed. |
