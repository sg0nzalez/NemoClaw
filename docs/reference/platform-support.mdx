...
| macOS (Apple Silicon) | Colima, Docker Desktop | Tested with limitations | P0 | Yes | Start the container runtime (Colima or Docker Desktop) before running the installer. OpenShell is installed through Homebrew and the gateway should appear in `brew services list` as `openshell`. Homebrew Colima users must install both Colima and the Docker CLI (`brew install colima docker`) before `docker info` can work. Xcode Command Line Tools (`xcode-select --install`) are typically required for Node native modules during install. NemoClaw recommends them but does not enforce them during preflight. |
...
| Intel Mac (macOS x86_64) | Unsupported | The supported macOS OpenShell Homebrew gateway service path is Apple Silicon only. Install hard-fails on x86_64 macOS. See issue #954 (closed). |
