...
| Rootless Podman gateway runtime | Experimental | Linux-only opt-in POC. Set `NEMOCLAW_GATEWAY_RUNTIME=podman` and run with rootless Podman, cgroups v2, subordinate UID/GID ranges, and a reachable Podman API socket. Onboarding can apply the package install, user socket, and subordinate ID setup when authorized; cgroups v2 remains a manual host/reboot prerequisite. Docker remains the default runtime. Sandbox GPU passthrough is not part of the Podman POC; use `--no-gpu` or `NEMOCLAW_SANDBOX_GPU=0`. |
...
| Item | Status | Why |
|------|--------|-----|
| Rootful Podman / other container runtimes | Unsupported | The Podman path is limited to the explicit rootless gateway-runtime POC (`NEMOCLAW_GATEWAY_RUNTIME=podman`). Rootful Podman, containerd, CRI-O, and other local runtimes are unsupported. Docker Engine, Docker Desktop, and Colima remain the tested default paths. |
