...
For tarball-installed OpenShell binaries, the installer and onboarding stage a marked user-level `openshell-gateway.service` that follows OpenShell's systemd service model, so the gateway can restart after reboot or at the next user session.
...
Arbitrary per-user units, partial units, and user-manager or bus outages do not take over gateway ownership; NemoClaw falls back to the standalone gateway process when no trusted service path is available.
If NemoClaw cannot write or remove its marked user unit, it reports the filesystem failure and keeps the standalone fallback available.
...
The installer installs OpenShell through the upstream Homebrew formula, and onboard restarts the `openshell` Homebrew service before checking gateway health.
If the Homebrew formula or service is unavailable, NemoClaw can still use the standalone Docker-driver gateway process as a compatibility fallback.
