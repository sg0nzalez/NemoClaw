...
The in-container `/tmp/nemoclaw-gateway-local` marker is created only by entrypoint supervisors that actually launch an in-container gateway and normally remains while that supervisor is active.
On normal exits, handled `SIGTERM`/`SIGINT`, and startup failures, the supervisor removes the marker on a best-effort basis so the Docker health check does not keep trusting a stale gateway PID.
