...
On Linux, NemoClaw configures, enables, and restarts a trusted OpenShell gateway user service, then creates the sandbox as a Docker container.
...
NemoClaw does not install or remove that Homebrew service; tarball-installed OpenShell binaries continue to use the standalone Docker-driver gateway process.
