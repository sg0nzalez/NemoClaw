...
On Linux, NemoClaw configures and restarts a trusted OpenShell gateway user service, then creates the sandbox as a Docker container.
...
For tarball-installed OpenShell binaries, NemoClaw stages a marked user-level `openshell-gateway.service` that follows OpenShell's systemd service model, so the gateway can restart after reboot or at the next user session.
