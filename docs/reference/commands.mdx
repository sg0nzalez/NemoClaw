...
Before creating the gateway, the wizard asks interactive users which gateway runtime to use, then runs preflight checks for that runtime.
Docker is the default gateway runtime.
Set `NEMOCLAW_GATEWAY_RUNTIME` to skip the prompt in scripted runs.
By default it verifies that Docker is reachable and prints host remediation guidance when prerequisites are missing.
When `NEMOCLAW_GATEWAY_RUNTIME=podman` is set, it instead verifies the experimental rootless Podman gateway-runtime prerequisites.
On the Podman path, onboarding can remediate missing safe prerequisites after authorization.
If the Podman package, user `podman.socket`, or subordinate UID/GID ranges are missing, interactive onboarding prints the planned commands and asks before running them.
If the socket file exists but does not accept requests, onboarding restarts `podman.socket` and asks you to verify `podman --url unix://$OPENSHELL_PODMAN_SOCKET info`.
Passing `--yes` authorizes the same setup commands for scripted POC runs.
Cgroups v2 is still a manual host and reboot prerequisite.
...
| `NEMOCLAW_GATEWAY_RUNTIME` | `docker` (default) or `podman` | Selects the OpenShell gateway runtime during onboarding and skips the interactive runtime prompt. `podman` enables the experimental rootless Linux Podman POC and requires rootless Podman, cgroups v2, subordinate UID/GID ranges, and a reachable Podman API socket. Onboarding can apply the package install, user socket, and subordinate ID setup when authorized; cgroups v2 remains a manual host/reboot prerequisite. Pair it with `--no-gpu` or `NEMOCLAW_SANDBOX_GPU=0` because sandbox GPU passthrough is not supported for the POC. |
| `OPENSHELL_PODMAN_SOCKET` | Podman API socket path, optionally with `unix://` prefix | Overrides Podman API socket auto-discovery when `NEMOCLAW_GATEWAY_RUNTIME=podman` is set. |
