...
Use `--json` for machine-readable CPU, memory, GPU, and profile data.
...
The cleanup selects one OpenShell mutation transport and dispatches the deletion once. It does not fall back to SSH or replay the deletion when the result is inconclusive, because the first dispatch may already have removed the state. If the sandbox is stopped, its gateway is unreachable, or success cannot be confirmed, the command refuses to proceed to the rebuild and asks you to start the sandbox and re-run `channels remove`. Host-side channel state stays unchanged on that failure path, so the re-run can reconcile the removal without restoring stale Baileys auth files.
...
A running sandbox whose OpenShell sandbox exec endpoint does not answer fails its backup and aborts the run.
...
uninstaller, and developer tooling. They are not part of the
...
| `NEMOCLAW_DISABLE_INFERENCE_ROUTE_REPAIR` | `1` to enable | Skips automatic route recovery for stale `inference.local` routes during `$$nemoclaw <name> connect` and `$$nemoclaw <name> connect --probe-only`. Use only as a troubleshooting escape hatch. |
...
| `NEMOCLAW_SKIP_UNREACHABLE_SANDBOX_BACKUP` | Exactly `1` to opt in (`true`, `yes`, `0` are not accepted) | Applies to standalone `$$nemoclaw backup-all` runs. Skips running sandboxes whose OpenShell sandbox exec endpoint does not answer. It does not relax the installer's strict pre-upgrade backup, which still aborts if any registered sandbox is skipped or fails. Any uncommitted state since the last successful backup is not included in the skipped backup. |
