...
Pass `--tools` with one server name to make a live MCP connection through the selected agent's policy, credential placeholder, proxy environment, and adapter runtime ancestry.
The shared client performs `initialize`, `notifications/initialized`, and paginated `tools/list`, then attempts to close the MCP session and transport.
...
Tool discovery is opt-in network traffic.
...
| `--tools` | Discover advertised tool names for one named server; suppresses the implicit credential probe unless `--probe` is also passed |
