...
It also bypasses locally recorded sandbox base-image resolution metadata and performs the normal pull-and-resolve flow.
`--fresh` takes precedence over a base-image hint carried from a rebuild, so NemoClaw performs a full pull-and-resolve flow.
...
Set `NEMOCLAW_SANDBOX_BASE_IMAGE_REFRESH=1` to bypass this reuse without changing onboarding session handling:
...
Base-image resolution controls apply in this order:

1. `--fresh` starts a new onboarding session and forces normal pull-and-resolve behavior.
2. `NEMOCLAW_SANDBOX_BASE_IMAGE_REFRESH=1` forces pull-and-resolve without discarding session state.
3. A recorded rebuild hint is reused only after local image identity, platform, digest, and ABI validation.
4. Without a usable hint, NemoClaw follows the normal published-image and local-fallback resolution path.

Managed LangChain Deep Agents Code rebuilds use a separate safety path.
NemoClaw builds the replacement from a pinned base and fingerprints the retained build context before it deletes the current sandbox.
That path does not reuse recorded base-image resolution metadata, so `NEMOCLAW_SANDBOX_BASE_IMAGE_REFRESH` does not change a Deep Agents Code rebuild.
