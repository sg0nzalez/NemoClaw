...
| `--keep-openshell` | Leave OpenShell binaries and gateway service daemons in place, without disabling or removing the OpenShell gateway service or stopping the host `openshell-gateway` daemon; a full uninstall can remove only NemoClaw's marked Linux `~/.config/systemd/user/openshell-gateway.service` unit, and upstream OpenShell services, including Linux package units and the macOS Homebrew service, are preserved. |
...
| `NEMOCLAW_OPENSHELL_GATEWAY_BIN` | path | Advanced override for the `openshell-gateway` binary used by Linux Docker-driver gateway startup, including the marked user service and standalone fallback. Defaults to the binary next to `openshell`, then common install paths. |
