...
| `NEMOCLAW_OPENSHELL_GATEWAY_BIN` | path | Advanced override for the `openshell-gateway` binary used by Docker-driver gateway startup when NemoClaw stages its Linux marked user service or uses the standalone fallback. The marked user service accepts only NemoClaw's trusted install paths (`~/.local/bin/openshell-gateway`, `/usr/local/bin/openshell-gateway`, or `/usr/bin/openshell-gateway`); other absolute paths use the standalone fallback. The macOS Homebrew service uses the Homebrew formula's gateway binary. Defaults to the binary next to `openshell`, then common install paths. |
