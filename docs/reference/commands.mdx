...
| `--keep-openshell` | Leave OpenShell binaries and gateway service daemons in place, without disabling or removing the OpenShell gateway service or stopping the host `openshell-gateway` daemon. This includes NemoClaw's marked Linux `openshell-gateway.service` user unit, upstream Linux package units, and the macOS Homebrew service. |
