...
OpenShell rejects provider removal while any sandbox still has that provider attached.
Destroy the affected sandbox, or detach the provider with `openshell sandbox provider detach <sandbox-name> <provider>`, before resetting the credential.
