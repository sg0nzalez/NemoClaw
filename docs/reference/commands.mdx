...
| `--keep-openshell` | Leave OpenShell binaries, gateway service daemons, and gateway service configuration in place, without disabling or removing the OpenShell gateway service or stopping the host `openshell-gateway` daemon. This includes NemoClaw's marked Linux `~/.config/systemd/user/openshell-gateway.service` unit, upstream Linux package units, and the macOS Homebrew service. |
...
| `NEMOCLAW_OPENSHELL_GATEWAY_BIN` | path | Advanced override for the `openshell-gateway` binary used by Linux Docker-driver gateway startup. The marked user service accepts only NemoClaw's trusted install paths (`~/.local/bin/openshell-gateway`, `/usr/local/bin/openshell-gateway`, or `/usr/bin/openshell-gateway`); other absolute paths use the standalone fallback. Defaults to the binary next to `openshell`, then common install paths. |
| `NEMOCLAW_OPENSHELL_SANDBOX_BIN` | path | Advanced override for the `openshell-sandbox` binary used by Linux Docker-driver gateway startup, including the marked user service environment and standalone fallback. Defaults to the binary next to `openshell`, then common install paths. |
