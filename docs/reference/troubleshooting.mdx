...
1. Check the managed OpenShell gateway service on Linux tarball installs.

   NemoClaw installs a marked user `openshell-gateway.service` on Linux tarball installs.
   ...
   On macOS, non-systemd Linux hosts, or hosts where the user service cannot be reached, continue with the NemoClaw recovery step below.
