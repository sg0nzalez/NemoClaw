...
When OpenShell creates the sandbox but it fails before reaching Ready, NemoClaw removes the failed sandbox so the next onboarding attempt can reuse the same name.
...
For a running OpenClaw or Hermes sandbox, inspect the authoritative OpenShell startup events:

```bash
openshell logs <sandbox-name> -n 100 --source sandbox
```

For Deep Agents Code, use Linux 5.19 or later with Landlock enabled, ensure the sandbox runtime permits Landlock syscalls, and correct any missing or inaccessible path reported by OpenShell.
