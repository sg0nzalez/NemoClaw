...
### Landlock kernel requirement blocks onboarding

Before sandbox creation or reuse, NemoClaw checks whether the host kernel supports Landlock (Linux 5.13+).
If the kernel is too old or you are running on macOS (where the Docker VM kernel may lack Landlock), onboarding aborts:
...
This check prevents a new sandbox from being created with reduced filesystem isolation.
Run on a Linux kernel 5.13 or later with Landlock enabled (Ubuntu 22.04 LTS and later include Landlock support), then retry onboarding.
