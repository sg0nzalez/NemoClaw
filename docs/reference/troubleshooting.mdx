...
1. Check the managed OpenShell gateway service.
   ...
   On Apple Silicon macOS, OpenShell is installed through Homebrew and the gateway is a Homebrew service:
   ...
   On Linux tarball installs, NemoClaw installs a marked user `openshell-gateway.service`.
   The unit is written under `$XDG_CONFIG_HOME/systemd/user` when `XDG_CONFIG_HOME` is absolute, otherwise under `~/.config/systemd/user`.
   The service generates the local OpenShell mTLS bundle before starting and then reads NemoClaw's `openshell/gateway.env` overrides.
   The systemd user manager starts it after reboot, or at the next user session when lingering is not enabled.
   If `systemctl --user` is available, check and restart it first:
   ...
   On non-systemd Linux hosts or hosts where the user service cannot be reached, continue with the NemoClaw recovery step below.
