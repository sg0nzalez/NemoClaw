...
This page encodes the two dollar signs as character entities so its own generated variants can display the literal placeholder instead of resolving it.
...
When an `<AgentOnly>` directive lists the active variant, the resolver copies the body into the generated page.
...
This guide has the following mapping:
...
An OpenClaw navigation entry for this guide uses the following shape:
