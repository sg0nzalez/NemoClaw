...
export class SyntheticMcpServer {
  ...
  constructor(catalog: SyntheticCatalog, bearerToken: string) {
    if (!bearerToken || bearerToken.length > 4_096) {
      throw new Error("benchmark MCP bearer token must be non-empty and bounded");
    }
    ...
    if (this.toolsByName.size !== catalog.size) {
      throw new Error("benchmark MCP catalog contains duplicate tool names");
    }
    this.server = http.createServer((request, response) => {
      void this.handle(request, response).catch(() => {
        jsonResponse(response, 500, { error: "benchmark MCP server failure" });
      });
    });
  ...
  async start(port = 0): Promise<SyntheticMcpServerAddress> {
    if (!Number.isInteger(port) || port < 0 || port > 65_535) {
      throw new Error("benchmark MCP port must be between 0 and 65535");
    }
    ...
    if (!address || address.address !== "127.0.0.1") {
      await this.stop();
      throw new Error("benchmark MCP server failed to bind loopback");
    }
    return {
      host: "127.0.0.1",
      port: address.port,
  ...
  beginRun(runId: string): void {
    if (!SAFE_RUN_ID.test(runId)) throw new Error("invalid benchmark MCP run id");
    if (this.activeRun) throw new Error("a benchmark MCP run is already active");
    this.activeRun = { id: runId, sequence: 0, eventStartIndex: this.events.length };
  }

  endRun(): SyntheticMcpCallEvent[] {
    const run = this.activeRun;
    if (!run) throw new Error("no benchmark MCP run is active");
    this.activeRun = null;
    return this.events
    ...
    if (payload.method === "initialize") {
      ...
      jsonResponse(response, 200, {
        ...
        result: {
          ...
          capabilities: { tools: {} },
          serverInfo: { name: "nemoclaw-tool-disclosure-benchmark", version: "1.0.0" },
        },
      });
      return;
    }
    ...
    if (payload.method === "tools/call") {
      ...
      if (!tool || !argumentsValue) {
        ...
        jsonResponse(response, 200, {
          jsonrpc: "2.0",
          id,
          error: { code: -32602, message: "invalid benchmark tool call" },
        });
        return;
      }
      try {
        ...
        jsonResponse(response, 200, {
          jsonrpc: "2.0",
          id,
          error: { code: -32602, message: "invalid benchmark tool arguments" },
        });
      }
      return;
    }
