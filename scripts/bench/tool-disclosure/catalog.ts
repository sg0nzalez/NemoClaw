...
export const TOOL_DISCLOSURE_CATALOG_SCHEMA_VERSION =
  "nemoclaw.tool_disclosure.catalog.v1" as const;
export const DEFAULT_SYNTHETIC_CATALOG_SEED = "nemoclaw-tool-disclosure-v1";
...
export interface SyntheticCatalog {
  schema_version: typeof TOOL_DISCLOSURE_CATALOG_SCHEMA_VERSION;
  seed: string;
  size: number;
...
// definition is intentionally near the benchmark's roughly 400-token target;
...
function buildParameters(
  ...
  return {
    type: "object",
    description: `Inputs for a synthetic ${category.label} benchmark operation.`,
    properties,
    required,
    additionalProperties: false,
  };
...
function generateTool(
  ...
  const name = `bench_${category.id}_${operation}_${index.toString().padStart(4, "0")}`;
    ...
    definition: {
      ...
      function: {
        ...
        description: [
          ...
          `Use for benchmark requests about ${category.purpose}; it never contacts a real service.`,
        ].join(" "),
...
export function generateSyntheticTools(
  ...
  seed: string | number = DEFAULT_SYNTHETIC_CATALOG_SEED,
): SyntheticTool[] {
...
export function generateSyntheticCatalog(options: SyntheticCatalogOptions = {}): SyntheticCatalog {
  ...
  const seed = normalizeSeed(options.seed ?? DEFAULT_SYNTHETIC_CATALOG_SEED);
  const tools = generateSyntheticTools(size, seed);
  return {
    schema_version: TOOL_DISCLOSURE_CATALOG_SCHEMA_VERSION,
    seed,
    size,
...
export function validateSyntheticCatalog(catalog: SyntheticCatalog): string[] {
  const errors: string[] = [];
  if (catalog.schema_version !== TOOL_DISCLOSURE_CATALOG_SCHEMA_VERSION) {
