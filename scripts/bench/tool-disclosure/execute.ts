...
import type { SyntheticBenchmarkTask, SyntheticTaskSet } from "./tasks";
import {
  countTokensWithVllm,
...
async function configureMcpSandbox(options: {
  ...
  if (options.agent === "hermes") {
    await run([
      ...
      "15m",
      "--reason",
      "tool disclosure benchmark setup",
    ]);
  }
  try {
    ...
    if (/"(?:name|server)"\s*:\s*"benchmark"/u.test(listed.stdout)) {
      await run([options.sandboxName, "mcp", "remove", "benchmark", "--force"]);
    }
    await run([
      options.sandboxName,
      "mcp",
...
): Map<string, SyntheticBenchmarkTask> {
...
export async function executeCampaign(options: {
  ...
  if (new Set(names).size !== names.length)
    throw new Error("every benchmark cell needs a unique sandbox");
  ...
  if (instanceIds.some((value) => !value) || new Set(instanceIds).size !== instanceIds.length) {
    throw new Error("every benchmark cell needs a unique live sandbox instance ID");
  }
  ...
  if (
    ...
    throw new Error("every benchmark cell needs a unique live container name");
  }
  ...
  const tokenEnv = "NEMOCLAW_BENCHMARK_MCP_BEARER";
