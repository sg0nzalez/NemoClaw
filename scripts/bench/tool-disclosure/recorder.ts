 ...
 * suitable for direct inclusion in the public benchmark evidence bundle.
 */
...
/**
 * Loopback-only OpenAI-compatible recording proxy for benchmark measurements.
  ...
  async start(): Promise<RecordingProxyAddress> {
    ...
    const server = http.createServer((request, response) => {
      void this.handleRequest(request, response).catch(() => {
        ...
        // evidence boundaries. The benchmark runner receives a fixed class.
  ...
  private async handleRequest(request: IncomingMessage, response: ServerResponse): Promise<void> {
    ...
    try {
      ...
      if (
        ...
        fixedResponse(response, 400, "benchmark temperature mismatch");
        this.finalizeRecording(pending, {
          statusCode: 400,
          outcome: "request-rejected",
