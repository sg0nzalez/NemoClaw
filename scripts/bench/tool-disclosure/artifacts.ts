...
export function ensureCampaignDirectory(outputDir: string, resume: boolean): string {
  ...
  if (fs.existsSync(resolved)) {
    ...
    if (stat.isSymbolicLink() || !stat.isDirectory()) {
      throw new Error("benchmark output path must be a real directory");
    }
    const entries = fs.readdirSync(resolved);
    if (!resume && entries.length > 0) {
      throw new Error("benchmark output directory is not empty; pass --resume to reuse it");
    }
  } else {
...
export function artifactPath(outputDir: string, name: string): string {
  if (!SAFE_ARTIFACT_NAME.test(name)) throw new Error(`invalid benchmark artifact name: ${name}`);
  const root = path.resolve(outputDir);
  const candidate = path.resolve(root, name);
  if (path.dirname(candidate) !== root)
    throw new Error("benchmark artifact escaped output directory");
  if (fs.existsSync(candidate) && fs.lstatSync(candidate).isSymbolicLink()) {
    throw new Error(`benchmark artifact must not be a symlink: ${name}`);
  }
  return candidate;
}
...
export function scanArtifactsForForbiddenValues(
  ...
  for (const name of names) {
    ...
    if (leaked) throw new Error(`benchmark artifact ${name} contains a forbidden value`);
  }
}
