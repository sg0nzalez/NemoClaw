...
export const TOOL_DISCLOSURE_PERFORMANCE_TEST_TASK_SCHEMA_VERSION =
  "nemoclaw.tool_disclosure.performance_test.tasks.v1" as const;
...
export type PerformanceTestTaskSuite = "primary" | "stress";
...
export interface SyntheticPerformanceTask {
  id: string;
...
export interface SyntheticTaskSet {
  schema_version: typeof TOOL_DISCLOSURE_PERFORMANCE_TEST_TASK_SCHEMA_VERSION;
  catalog_seed: string;
  suite: PerformanceTestTaskSuite;
  task_count: number;
  tasks_sha256: string;
  tasks: SyntheticPerformanceTask[];
}
...
function buildNoToolTasks(): SyntheticPerformanceTask[] {
...
export function buildPrimaryTasks(catalog: SyntheticCatalog): SyntheticPerformanceTask[] {
...
export function buildStressTasks(catalog: SyntheticCatalog): SyntheticPerformanceTask[] {
...
function buildTaskSet(
  ...
  return {
    schema_version: TOOL_DISCLOSURE_PERFORMANCE_TEST_TASK_SCHEMA_VERSION,
...
function expectedKindCounts(
  suite: PerformanceTestTaskSuite,
): Readonly<Record<PerformanceTestTaskKind, number>> {
...
export function validateSyntheticTaskSet(
  ...
  const errors: string[] = [];
  if (taskSet.schema_version !== TOOL_DISCLOSURE_PERFORMANCE_TEST_TASK_SCHEMA_VERSION) {
  ...
  const counts: Record<PerformanceTestTaskKind, number> = {
    "single-tool": 0,
    "ordered-chain": 0,
    "near-match": 0,
    "no-tool": 0,
  ...
  for (const kind of Object.keys(counts) as PerformanceTestTaskKind[]) {
