...
export const OPENCLAW_PERFORMANCE_TEST_CALLS_PATH =
  "/sandbox/.nemoclaw-performance-test/calls.jsonl";
...
export function buildOpenClawCallLogCommand(options: {
  ...
  const script =
    options.action === "reset"
      ? `rm -f -- '${OPENCLAW_PERFORMANCE_TEST_CALLS_PATH}'`
      : `[ ! -f '${OPENCLAW_PERFORMANCE_TEST_CALLS_PATH}' ] || cat -- '${OPENCLAW_PERFORMANCE_TEST_CALLS_PATH}'`;
  return {
    command: options.openshellBin ?? "openshell",
    args: ["sandbox", "exec", "-n", options.sandboxName, "--", "sh", "-lc", script],
    redactions: [],
