...
const PHASE_ORDER: Readonly<Record<Exclude<PerformanceTestPhase, "static-visibility">, number>> = {
  "small-control": 0,
  primary: 1,
  "large-stress": 2,
};
...
function validateEvidenceSet(
  ...
  if (manifest.schema_version !== TOOL_DISCLOSURE_PERFORMANCE_TEST_SCHEMA_VERSION) {
  ...
  for (const run of runs) {
    ...
    if (run.performance_test_id !== manifest.performance_test_id) {
      throw new Error(`run ${run.run_id} belongs to a different performance test`);
    }
  }
}

function validateRun(run: ToolDisclosureRun): void {
  if (run.schema_version !== TOOL_DISCLOSURE_PERFORMANCE_TEST_SCHEMA_VERSION) {
