...
function usage(): string {
  return `Progressive tool-disclosure performance test\n\n\
Usage:\n\
...
function parseArguments(args: readonly string[]): ParsedArguments {
  ...
  const parsed: ParsedArguments = {
    command,
    sutRef: "HEAD",
    catalogSeed: DEFAULT_SYNTHETIC_PERFORMANCE_TEST_CATALOG_SEED,
...
function performanceTestId(sha: string, catalogHash: string): string {
...
function manifestTemplate(options: {
  ...
  return {
    ...
    performance_test_id: performanceTestId(sut.git_sha, options.catalogHash),
...
function prepare(options: ParsedArguments): void {
  ...
  if (
    ...
    throw new Error("performance test output directory must be outside the Git worktree");
  }
  const worktreeClean = gitOutput(["status", "--porcelain=v1", "--untracked-files=normal"]) === "";
  if (!worktreeClean && !options.allowDirty) {
    throw new Error(
      "performance test preparation requires a clean worktree; commit or stash changes first",
    );
  }
...
function assertPreparedIdentity(
  ...
  if (
    manifest.performance_test_id !== performanceTestId(manifest.sut.git_sha, catalog.tools_sha256)
  ) {
    throw new Error("manifest performance test ID does not match the frozen SUT and catalog");
  }
...
function summarize(options: ParsedArguments): void {
  ...
  const bundle: EvidenceBundle = {
    schema_version: TOOL_DISCLOSURE_PERFORMANCE_TEST_SCHEMA_VERSION,
    performance_test_id: manifest.performance_test_id,
...
if (import.meta.url === `file://${process.argv[1]}`) {
  void main().catch((error: unknown) => {
    process.stderr.write(
      `tool-disclosure performance test: ${error instanceof Error ? error.message : String(error)}\n`,
    );
    process.exitCode = 1;
  });
}
