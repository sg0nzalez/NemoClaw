...
readarray -t package_identity < <(
  PACKAGE_JSON="$package_json" PACKAGE_LOCK="$package_lock" node <<'NODE'
...
if (typeof plugin.resolved !== "string" || !plugin.resolved.startsWith("https://registry.npmjs.org/")) {
  throw new Error("WeChat runtime plugin must resolve from the reviewed npm registry origin");
}
