...
import {
  ...
  validateSandboxBaseImageResolutionMetadata,
  versionGte,
...
describe("sandbox base image helpers", () => {
  const publishedMetadata: SandboxBaseImageResolutionMetadata = {
    schema: 1,
    key: "resolution-key",
  ...
  it("validates a published warm-cache resolution from local Docker metadata (#4680)", () => {
    expect(
      validateSandboxBaseImageResolutionMetadata({
        metadata: publishedMetadata,
        expectedKey: "resolution-key",
        imageName: publishedMetadata.imageName,
        requireOpenshellSandboxAbi: true,
        minGlibcVersion: "2.39",
        inspected: {
          Id: "sha256:image-id",
          Os: "linux",
          Architecture: "amd64",
          RepoDigests: [`${publishedMetadata.imageName}@sha256:abc`],
        },
      }),
    ).toEqual({ ok: true });
  });

  it("rejects a published hint when RepoDigests no longer proves the pinned digest (#4680)", () => {
    expect(
      validateSandboxBaseImageResolutionMetadata({
        metadata: publishedMetadata,
        expectedKey: "resolution-key",
        ...
        inspected: {
          ...
          Os: "linux",
          Architecture: "amd64",
          RepoDigests: [`${publishedMetadata.imageName}@sha256:different`],
        },
      }),
    ).toEqual({ ok: false, reason: "repo_digest_missing" });
  });

  it("validates local fallback images by image identity without RepoDigests (#4680)", () => {
    const localMetadata: SandboxBaseImageResolutionMetadata = {
      ...publishedMetadata,
    ...
    expect(
      validateSandboxBaseImageResolutionMetadata({
        ...
        expectedKey: "resolution-key",
        imageName: localMetadata.imageName,
        requireOpenshellSandboxAbi: true,
        minGlibcVersion: "2.39",
        inspected: {
          Id: "sha256:image-id",
  ...
  it("rejects stale keys, platform drift, and incompatible ABI evidence (#4680)", () => {
    ...
    ).toEqual({ ok: false, reason: "local_image_changed" });
    expect(
      validateSandboxBaseImageResolutionMetadata({
        metadata: { ...publishedMetadata, glibcVersion: "2.36" },
        expectedKey: "resolution-key",
        imageName: publishedMetadata.imageName,
        requireOpenshellSandboxAbi: true,
        minGlibcVersion: "2.39",
        inspected: { ...inspected, Architecture: "amd64" },
      }),
    ).toEqual({ ok: false, reason: "abi_incompatible" });
  });

  it("formats completed-image labels with round-trippable resolution metadata (#4680)", () => {
    const labels = formatSandboxBaseImageResolutionLabels(publishedMetadata);
    const encoded = labels.match(/base-resolution="([^"]+)"/)?.[1];
    expect(labels).toContain(`base-resolution-key="${publishedMetadata.key}"`);
  ...
  it("treats missing or corrupt completed-image resolution labels as a cache miss (#4680)", () => {
    expect(parseSandboxBaseImageResolutionLabels(null)).toBeNull();
    expect(
      parseSandboxBaseImageResolutionLabels({
        "com.nvidia.nemoclaw.base-resolution": "not-valid-base64-json",
      }),
    ).toBeNull();
  });

  it("invalidates the resolution key when a relevant base input changes (#4680)", () => {
    ...
    const before = createSandboxBaseImageResolutionKey(options);
    writeFixture(root, "Dockerfile.base", "FROM node:22\nRUN echo changed\n");
    expect(createSandboxBaseImageResolutionKey(options)).not.toBe(before);
  });

  it("isolates explicit base-image overrides in the resolution key (#4680)", () => {
    const root = createGitFixture();
    const options = {
      imageName: "ghcr.io/nvidia/nemoclaw/sandbox-base",
      dockerfilePath: path.join(root, "Dockerfile.base"),
    ...
    const first = createSandboxBaseImageResolutionKey({
      ...
      env: {
        ...gitEnv,
        NEMOCLAW_SANDBOX_BASE_IMAGE_REF: "example/base@sha256:first",
      },
    });
    const second = createSandboxBaseImageResolutionKey({
      ...
      env: {
        ...gitEnv,
        NEMOCLAW_SANDBOX_BASE_IMAGE_REF: "example/base@sha256:second",
      },
    });
