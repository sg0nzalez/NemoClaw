...
describe("rebuild agent base image preflight", () => {
  ...
  it("forces a repository-local build and returns its exact ref for a legacy sandbox without a hint (#4680)", () => {
  ...
  it("uses a recorded resolution hint for cache validation without forcing a rebuild (#4680)", () => {
    const imageRef = "nemoclaw-hermes-sandbox-base-local:cached";
  ...
  it("forwards force refresh with the recorded resolution hint (#4680)", () => {
    const imageRef = "nemoclaw-hermes-sandbox-base-local:refreshed";
    const hint = { key: "sandbox-a" } as SandboxBaseImageResolutionMetadata;
    const { ensureAgentBaseImage } = mockBaseImagePreflight(imageRef);
    const { ensureRebuildAgentBaseImage } = loadRebuildFlowHelpers();
