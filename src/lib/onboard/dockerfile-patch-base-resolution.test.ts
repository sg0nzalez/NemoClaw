...
function temporaryDockerfile(content: string): string {
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), "nemoclaw-base-resolution-label-test-"));
...
afterEach(() => {
  for (const directory of tmpRoots.splice(0)) {
    fs.rmSync(directory, { recursive: true, force: true });
  }
});

describe("Dockerfile base-resolution metadata", () => {
  it("stamps base-resolution metadata on the completed managed image (#4680)", () => {
    const dockerfilePath = temporaryDockerfile("FROM scratch\n");
    const metadata = {
      schema: 1,
      key: "resolution-key",
