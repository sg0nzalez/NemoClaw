...
vi.mock("./sandbox-create-failure", async (importOriginal) => ({
  ...(await importOriginal<typeof import("./sandbox-create-failure")>()),
...
describe("runSandboxGpuCreateFlow native failure and readiness", () => {
  ...
  it("does not delete a same-name sandbox from hard-required Landlock create output (#5795)", async () => {
    mocks.streamSandboxCreate.mockResolvedValueOnce({
      ...
      output: [
        ...
        "Landlock filesystem sandbox unavailable (hard_requirement, will fail)",
        "Failed to prepare sandbox: incompatible directory-only access-rights: ReadDir",
      ].join("\n"),
      sawProgress: true,
    });
    const deps = createDeps();
    const exit = mockExit(17);
  ...
  it("does not delete a same-name sandbox without exact create evidence (#5795)", async () => {
    mocks.streamSandboxCreate.mockResolvedValueOnce({
      status: 17,
      output: "Landlock unavailable in hard_requirement mode: not enabled in the active LSM set",
      sawProgress: false,
    });
    const deps = createDeps();
    mockExit(17);
