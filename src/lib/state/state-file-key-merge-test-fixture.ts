...
import { KEY_ALLOWLIST_MERGE_PYTHON, stateFileKeyMergeSpec } from "./state-file-key-merge";
...
export function runMergeScript(
  ...
  const dir = fs.realpathSync(
    fs.mkdtempSync(path.join(os.tmpdir(), "nemoclaw-state-file-key-merge-")),
  );
  try {
    const currentPath = path.join(dir, "config.toml");
    const stagedPath = path.join(dir, "nemoclaw-state-restore-backup");
    fs.writeFileSync(currentPath, current, { mode: 0o660 });
    fs.writeFileSync(stagedPath, backup, { mode: 0o600 });
...
export function runProductionCommand(
  ...
  const dir = fs.realpathSync(
    fs.mkdtempSync(path.join(os.tmpdir(), "nemoclaw-state-file-command-")),
  );
  ...
  try {
    const stagedPath = path.join(dir, "nemoclaw-state-restore-backup");
    fs.writeFileSync(stagedPath, backup, { mode: 0o600 });
    fs.chmodSync(stagedPath, options.stagedMode ?? 0o600);
    ...
    const args = [
      ...
      stagedPath,
      options.stagedDigest ?? sha256(backup),
    ];
    const command = JSON.stringify(["python3", ...args]);
