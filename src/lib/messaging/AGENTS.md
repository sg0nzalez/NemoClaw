...
This package owns NemoClaw's manifest-first messaging architecture. It turns channel declarations for Telegram, Discord, Slack, WeChat, WeCom, and WhatsApp into a serializable `SandboxMessagingPlan`, then applies that plan during onboard, channel add/remove/start/stop, rebuild, image build, runtime setup, diagnostics, and conflict checks.
...
Mock external messaging APIs. Do not call real Telegram, Discord, Slack, WeChat, WeCom, WhatsApp, NVIDIA, or OpenShell services from unit tests.
