...
describe("parseSandboxMessagingPlan", () => {
  ...
  it("normalizes versioned Hermes credential placeholders from full persisted plans", () => {
        ...
        agentRender: [
          {
            ...
            lines: [
              ...
              "DISCORD_BOT_TOKEN=openshell:resolve:env:v1442987827285932589_DISCORD_BOT_TOKEN",
            ],
            templateRefs: [],
          },
        ],
    ...
    expect(parsed?.credentialBindings[0]?.placeholder).toBe(
      "openshell:resolve:env:DISCORD_BOT_TOKEN",
    );
    expect(parsed?.agentRender[0]).toMatchObject({
      kind: "env-lines",
      lines: ["API_SERVER_PORT=18642", "DISCORD_BOT_TOKEN=openshell:resolve:env:DISCORD_BOT_TOKEN"],
    });
  });
