...
test("tool disclosure hosted-inference performance smoke completes one frozen task in both modes", {
  ...
  for (const mode of MODES) {
    ...
    const attestation = await sandbox.exec(
      name,
      ["nemoclaw-start", "sh", "-c", `test "$NEMOCLAW_TOOL_DISCLOSURE" = ${JSON.stringify(mode)}`],
      {
