...
function performanceSmokeJob(): WorkflowJob {
  const workflow = readWorkflow() as { jobs: Record<string, WorkflowJob> };
  const job = workflow.jobs["tool-disclosure-performance-smoke"];
...
describe("tool-disclosure performance smoke workflow boundary", () => {
  it("keeps the hosted-inference smoke explicit-only", () => {
    const inventory = readFreeStandingJobsInventory();

    expect(validateE2eWorkflowBoundary()).toEqual([]);
    expect(inventory.allowedJobs).toContain("tool-disclosure-performance-smoke");
    expect(inventory.explicitOnlyJobs).toContain("tool-disclosure-performance-smoke");
    expect(inventory.targetToJob.get("tool-disclosure-performance-smoke")).toBe(
    ...
    for (const selector of [
      { jobs: "tool-disclosure-performance-smoke" },
      { targets: "tool-disclosure-performance-smoke" },
    ]) {
      expect(evaluateE2eWorkflowDispatchSelectors(selector)).toMatchObject({
        valid: true,
        liveTargetsRun: false,
        selectedFreeStandingJobs: ["tool-disclosure-performance-smoke"],
        registryTargets: [],
      });
    }
  });
  ...
  it("runs the focused live test with reviewed prerequisites and scoped secrets", () => {
    ...
    expect(job).toMatchObject({
      ...
      env: {
        ...
        E2E_ARTIFACT_DIR:
          "${{ github.workspace }}/e2e-artifacts/live/tool-disclosure-performance-smoke",
    ...
    const run = namedStep(job, "Run tool-disclosure performance smoke live test");
    expect(run?.env).toEqual({
      NVIDIA_INFERENCE_API_KEY: "${{ secrets.NVIDIA_INFERENCE_API_KEY }}",
    });
    expect(run?.run).toContain("test/e2e/live/tool-disclosure-performance-smoke.test.ts");
    expect(run?.run).toContain("--project e2e-live");
  });

  it("maps only the smoke artifact directory through the reviewed uploader", () => {
    const job = performanceSmokeJob();
    const upload = namedStep(job, "Upload tool-disclosure performance smoke artifacts");
    expect(upload).toEqual({
      name: "Upload tool-disclosure performance smoke artifacts",
      if: "always()",
      uses: UPLOAD_E2E_ARTIFACTS_ACTION,
    });
    expect(job.env?.E2E_TARGET_ID).toBe("tool-disclosure-performance-smoke");
