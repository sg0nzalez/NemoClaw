...
`,
      ...
      const manifest = writeBackup("deepagents", "2026-07-06T10-00-00-000Z", {
        agentType: "langchain-deepagents-code",
        dir: "/sandbox/.deepagents",
        stateFiles: [{ path: "config.toml", strategy: "copy" }],
      });
      fs.writeFileSync(
        path.join(String(manifest.backupPath), "config.toml"),
        'model = "old-model"\n',
      );

      const restore = (sandboxState.restoreSandboxState as any)("deepagents", manifest.backupPath, {
