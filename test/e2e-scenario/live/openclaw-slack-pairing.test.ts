...
test.skipIf(!shouldRunLiveE2EScenarios())(
  ...
  async ({ artifacts, cleanup, host, sandbox, secrets, skip }) => {
    ...
    try {
      const install = await installSandbox(host, env, redactions, "install-slack-pairing");
      expectExitZero(install, "install.sh --non-interactive with Slack");
    } catch (error) {
      if (String(error).includes("NVIDIA_ENDPOINT_RATE_LIMIT")) {
        skip("NVIDIA endpoint validation was rate-limited before Slack pairing assertions ran");
        return;
      }
      throw error;
    }
