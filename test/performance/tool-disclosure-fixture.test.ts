...
import { assembleToolDisclosureRun } from "../../scripts/performance/tool-disclosure/assemble-run";
import {
  canonicalJson,
...
} from "../../scripts/performance/tool-disclosure/openclaw-fixture";
import { buildToolDisclosureSchedule } from "../../scripts/performance/tool-disclosure/schedule";
import { buildPrimaryTasks } from "../../scripts/performance/tool-disclosure/tasks";
import {
...
afterEach(() => {
  delete process.env.NEMOCLAW_PERFORMANCE_TEST_CALLS_PATH;
  for (const directory of temporaryDirectories.splice(0)) {
    fs.rmSync(directory, { recursive: true, force: true });
  }
});

describe("OpenClaw performance test fixture", () => {
  it("registers and executes the deterministic native tool catalog", async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "nemoclaw-performance-test-fixture-"));
    ...
    process.env.NEMOCLAW_PERFORMANCE_TEST_CALLS_PATH = callsPath;
...
describe("performance test grading", () => {
  ...
  it("assembles public-safe success and terminal run outcomes", () => {
    ...
    const manifest = {
      ...
      performance_test_id: "fixture-performance-test",
      campaigns: [
        {
