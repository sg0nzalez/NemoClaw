...
} from "../../scripts/performance/tool-disclosure/telemetry";

describe("vLLM performance test telemetry", () => {
...
vllm:prompt_tokens_total{model_name="performance-test",engine="0"} 100
vllm:prompt_tokens_total{model_name="performance-test",engine="1"} 20
vllm:generation_tokens_total{model_name="performance-test",engine="0"} 30
vllm:generation_tokens_total{model_name="performance-test",engine="1"} 2
`);
  ...
  it("rejects non-loopback HTTPS telemetry and tokenizer URLs for the local performance test", () => {
  ...
  it("fetches localhost metrics and tokenizer routes through literal IPv4 loopback", async () => {
    ...
    await expect(
      countTokensWithVllm("http://localhost:8000", "performance-test-model", "[]", tokenizerFetch),
    ).resolves.toBe(4);
