...
import {
  executeArgumentHash,
  SyntheticMcpServer,
} from "../../scripts/performance/tool-disclosure/mcp-server";
...
describe("synthetic performance test MCP server", () => {
  it("lists and executes the generated catalog while retaining content-free call evidence", async () => {
    ...
    try {
      ...
      expect((await initialized.json()) as object).toMatchObject({
        result: { serverInfo: { name: "nemoclaw-tool-disclosure-performance-test" } },
      });
