...
This runbook is the maintained runtime-validation path. Anyone reviewing #4851 acceptance can run it directly against `inference.nvidia.com` and confirm the model returns `content` with both file-creation code and the run command after the preload's system message is injected.
...
- An NVIDIA API key with access to `nvidia/nemotron-3-ultra-550b-a55b` (inference.nvidia.com → API Keys).
...
```bash
curl -sS -X POST https://inference.nvidia.com/v1/chat/completions \
...
- 2026-06-09 — verified by @cjagwani on a GCP Brev box against `inference.nvidia.com`. Numbers and content above match this run.
