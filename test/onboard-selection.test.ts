...
if echo "$url" | grep -q 'generativelanguage.googleapis.com' && echo "$url" | grep -q '/responses$'; then
  ...
  body='{"id":"chatcmpl-123","choices":[{"message":{"content":"OK"}}]}'
  status="200"
elif echo "$url" | grep -q 'inference.nvidia.com' && echo "$url" | grep -q '/responses$'; then
  body='{"id":"resp_123"}'
  status="200"
elif echo "$url" | grep -q 'inference.nvidia.com' && echo "$url" | grep -q '/chat/completions$'; then
  body='{"id":"chatcmpl-123","choices":[{"message":{"content":"OK"}}]}'
  status="200"
fi
printf '%s' "$body" > "$outfile"
...
`;
    ...
    assert.ok(payload.lines.every((line: string) => !line.includes("settings/api-keys")));
  });
...
(async () => {
  for (const key of ["NVIDIA_API_KEY", "NGC_API_KEY", "NEMOCLAW_PROVIDER_KEY"]) {
    delete process.env[key];
  }
  __setNonInteractive(true);
  const originalLog = console.log;
