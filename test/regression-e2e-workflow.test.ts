...
describe("Regression E2E workflow contract", () => {
  ...
  it("runs gateway drift preflight through Vitest artifacts", () => {
    const jobsDescription = workflow.on?.workflow_dispatch?.inputs?.jobs?.description ?? "";
    const selectorScript =
      workflow.jobs?.select_regression_jobs?.steps?.find((step) => step.id === "select")?.run ?? "";
    ...
    const runStep = job?.steps?.find(
      (step) => step.name === "Run gateway drift preflight Vitest E2E test",
    );
    ...
    const uploadStep = job?.steps?.find(
      (step) => step.name === "Upload gateway drift preflight artifacts",
    );

    expect(jobsDescription).toContain("gateway-drift-preflight-e2e");
    expect(selectorScript).toContain("gateway-drift-preflight-e2e");
    expect(selectorScript).toContain("gateway_drift_preflight=true");
    ...
    expect(runStep?.run).toContain("test/e2e-scenario/live/gateway-drift-preflight.test.ts");
    expect(runStep?.run).not.toContain("test/e2e/test-gateway-drift-preflight.sh");
    expect(runStep?.env?.NEMOCLAW_RUN_E2E_SCENARIOS).toBe("1");
    expect(runStep?.env?.E2E_ARTIFACT_DIR).toBe(
      "${{ github.workspace }}/e2e-artifacts/vitest/gateway-drift-preflight",
    );
    expect(uploadStep?.if).toBe("always()");
    expect(uploadStep?.with?.path).toBe("e2e-artifacts/vitest/gateway-drift-preflight/");
